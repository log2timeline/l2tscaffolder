...
class DefinitionManager:
    ...
    def DeregisterDefinition(
        ...
        if definition_name not in cls._definition_classes:
            raise KeyError(
                "Definition class not set for name: {0:s}.".format(definition_name)
            )

        del cls._definition_classes[definition_name]

    @classmethod
    def GetDefinitionByName(cls, name: str) -> Type[interface.ScaffolderDefinition]:
    ...
    @classmethod
    def RegisterDefinition(cls, definition_class: Type[interface.ScaffolderDefinition]):
        ...
        if definition_name in cls._definition_classes:
            raise KeyError(
                "Project class already set for name: {0:s}.".format(definition_name)
            )
