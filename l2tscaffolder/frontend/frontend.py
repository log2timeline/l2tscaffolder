...
class ScaffolderFrontend:
    ...
    def _AskDictQuestion(
        self, question: scaffolder_interface.DictQuestion
    ) -> Dict[str, str]:
    ...
    def _AskListQuestion(
        self, question: scaffolder_interface.ListQuestion
    ) -> List[str]:
        ...
        while more_entries:
            ...
            more_entries = self._output_handler.Confirm("Add more values?", abort=False)
            entry_index += 1
    ...
    def _AskStringQuestion(self, question: scaffolder_interface.StringQuestion) -> str:
    ...
    def _GetSelection(self, items: list, item_text: str) -> str:
        ...
        for item_count, item in enumerate(items):
            self._output_handler.PrintInfo("  [{0:d}] {1:s}".format(item_count, item))
        ...
        raise KeyError(
            "Item [{0:d}] not in list, please pick a valid number.".format(result_int)
        )
    ...
    def CreateGitFeatureBranch(self, project_path: str, module_name: str):
        ...
        self._output_handler.PrintOutput(
            "Switching to feature branch {0:s}".format(branch_name)
        )
    ...
    def GetDefinition(
        self, definition_string: str
    ) -> definition_interface.ScaffolderDefinition:
        ...
        """
        definitions = list(definition_manager.DefinitionManager.GetDefinitionNames())
    ...
    def GetProjectPath(
        ...
        check = self._output_handler.Confirm(
            ...
            ).format(project_path, definition.NAME)
        )

        if check:
            return self.GetProjectPath(definition)

        raise errors.WrongCliInput("Unable to proceed without a valid project path.")
