...
class TestTheuserEventData(events.EventData):
  """test theuser event data.
  ...
  def __init__(self):
    ...
    super(TestTheuserEventData, self).__init__(data_type=self.DATA_TYPE)
...
class TestPlugin(interface.SQLitePlugin):
  ...
  QUERIES = [((u'select * from users)'), u'ParseTheuserRow')]
  ...
  def ParseTheuserRow(self, parser_mediator, row, query=None, **unused_kwargs):
