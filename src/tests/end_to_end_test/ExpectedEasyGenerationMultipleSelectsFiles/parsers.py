...
class TestStatusesEventData(events.EventData):
  """test statuses event data.
  ...
  def __init__(self):
    """Initializes event data."""
    super(TestStatusesEventData, self).__init__(data_type=self.DATA_TYPE)
    self.card = None
...
class TestPlugin(interface.SQLitePlugin):
  ...
  QUERIES = [((u'select * from users)'), u'ParseUsersRow'), ((
      u'select * from statuses)'), u'ParseStatusesRow')]
