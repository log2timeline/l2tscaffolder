...
class TimesketchSketchScaffolderTest(unittest.TestCase):
    ...
    def testTimesketchSketchScaffolder(self):
        ...
        expected_init_files = frozenset(["timesketch/lib/analyzers/__init__.py"])
        init_generated = dict(scaffolder.GetInitFileChanges())
        self.assertEqual(set(init_generated.keys()), expected_init_files)

        expected_parser_init_addition = "from timesketch.lib.analyzers import testing\n"
        self.assertEqual(
        ...
        with open(
            "test_data/timesketch_sketch_testing_plugin.py", encoding="utf-8"
        ) as file_object:
